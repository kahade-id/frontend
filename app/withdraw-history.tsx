/**
 * Screen — Riwayat Penarikan (GET /v1/wallet/withdraw-history).
 */
...
export default function WithdrawHistoryScreen() {
  ...
  const fetch = useCallback(async (nextPage: number, refresh = false) => {
    ...
    setError(null)
    try {
      const res = await api.wallet.getWithdrawHistory({ page: nextPage, limit: PAGE_SIZE })
      const incoming = res.data ?? []
      setItems((prev) => (refresh || nextPage === 1 ? incoming : [...prev, ...incoming]))
      setPage(nextPage)
      setHasMore(nextPage < (res.meta?.totalPages ?? 1))
    } catch {
      if (nextPage === 1) setError("Gagal memuat riwayat penarikan.")
    } finally {
      setLoading(false)
    }
  }, [])
  ...
  return (
    <Screen edges={["top"]} padded={false}>
      <Header title="Riwayat Penarikan" />
      <PullToRefresh
