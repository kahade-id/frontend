...
export default function HeroSection() {
  ...
  return (
    <section className="section-padding-hero relative overflow-hidden">
      ...
      />
      {/* Gradient blobs — sized for mobile, larger on desktop */}
      <div
        className="absolute -top-10 right-0 w-[260px] h-[260px] sm:w-[380px] sm:h-[380px] lg:w-[520px] lg:h-[520px] rounded-full blur-2xl sm:blur-3xl opacity-50 sm:opacity-70 pointer-events-none"
        aria-hidden="true"
        style={{ background: 'radial-gradient(circle, var(--muted) 0%, transparent 70%)' }}
      />
      <div
        className="absolute -bottom-16 left-0 w-[200px] h-[200px] sm:w-[280px] sm:h-[280px] lg:w-[360px] lg:h-[360px] rounded-full blur-2xl sm:blur-3xl opacity-25 sm:opacity-40 pointer-events-none"
        aria-hidden="true"
        style={{ background: 'radial-gradient(circle, var(--muted) 0%, transparent 70%)' }}
      />
                ...
                {heroStats.map(({ value, label, icon: StatIcon }) => (
                  <div key={label} className="bg-muted rounded-xl p-3">
                    <div className="flex items-center gap-1.5 mb-1">
                      ...
                      <span className="text-[0.6875rem] font-semibold text-muted-foreground uppercase tracking-wider">{label}</span>
                    </div>
