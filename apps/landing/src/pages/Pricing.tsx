...
const pricingFaqs = [
  {
    ...
    a: 'Ya! Paket Pemula sepenuhnya gratis dan tidak memerlukan kartu kredit. Anda bisa mencoba Kahade tanpa risiko.',
  },
  {
    q: 'Bisakah saya ganti paket kapan saja?',
    a: 'Tentu. Anda bisa upgrade atau downgrade paket kapan saja. Perubahan berlaku di awal periode billing berikutnya.',
  },
  {
    q: 'Apakah biaya 2.5% bisa dinegosiasi?',
    a: 'Untuk paket Enterprise dengan volume tinggi, biaya platform bisa dinegosiasikan. Hubungi tim sales kami.',
  },
  {
    q: 'Apa yang terjadi jika melebihi batas transaksi?',
    a: 'Di paket Pemula, Anda akan diminta upgrade ke Profesional. Tidak ada biaya tambahan yang mengejutkan.',
  },
];
...
export default function Pricing() {
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <PageHero
        eyebrow="Harga"
        title="Harga yang Jelas dan Transparan"
        description="Mulai tanpa biaya awal, lalu pilih paket sesuai volume transaksi dan kebutuhan bisnis Anda."
        chips={[{ label: 'Tanpa Setup Fee' }, { label: 'Skalabel' }, { label: 'Transparan' }]}
      />
          ...
          <div className="grid md:grid-cols-3 gap-6 max-w-5xl mx-auto">
            {plans.map((plan) => {
              ...
              return (
                <div
                  key={plan.name}
                  className={`card p-8 relative flex flex-col ${plan.popular ? 'border-primary ring-1 ring-primary/15' : ''}`}
                >
                  {plan.popular && (
                          ...
                          <div>
                            ...
                            {isYearly && (
                              <div className="text-xs text-green-600 mt-1">
                                Hemat {formatRupiah((plan.monthlyPrice! - price) * 12)}/tahun
                              </div>
                            )}
                          </div>
                        )}
                  ...
                  <div className="space-y-3 mb-8 flex-1">
                    {plan.features.map((f) => (
                      <div key={f.text} className="flex items-center gap-3">
                        ...
                        <span className={`text-sm ${f.included ? '' : 'text-muted-foreground line-through'}`}>
                          {f.text}
                        </span>
                      </div>
                    ))}
                  ...
                  <Button asChild variant={plan.popular ? 'primary' : 'secondary'} className="w-full">
                    <a href="https://app.kahade.id/register">{plan.cta}</a>
                  </Button>
                </div>
              );
            })}
            ...
            <div className="bg-background rounded-3xl p-8 md:p-12 border border-border">
              <div className="grid md:grid-cols-2 gap-12 items-center">
                <div>
                  ...
                  <div className="mb-6">
                    ...
                    <div className="flex justify-between text-xs text-muted-foreground mt-1">
                      <span>Rp 500K</span>
                      <span>{formatRupiah(txValue)}</span>
                      <span>Rp 100M</span>
                    </div>
                  </div>
                ...
                <div className="space-y-4">
                  ...
                  <Button asChild variant="primary" className="w-full mt-2 inline-flex items-center justify-center">
              ...
              <div className="border border-border rounded-2xl overflow-hidden min-w-[480px]">
                <div className="grid grid-cols-4 bg-muted/50">
                  ...
                  {['Pemula', 'Profesional', 'Enterprise'].map((p) => (
                    <div
                      key={p}
                      className={`p-4 font-bold text-sm text-center ${
                        p === 'Profesional' ? 'bg-primary/5 border-x border-primary/20 text-primary' : ''
                      }`}
                    >
                      {p}
                    </div>
                ...
                {comparisonFeatures.map((row, i) => (
                  <div key={row.feature} className={`grid grid-cols-4 border-t border-border ${i % 2 === 0 ? '' : 'bg-muted/20'}`}>
                    <div className="p-4 text-sm text-muted-foreground">{row.feature}</div>
                    {[row.pemula, row.profesional, row.enterprise].map((val, j) => (
                      <div key={`${row.feature}-${j}`} className={`p-4 text-sm text-center ${j === 1 ? 'bg-primary/5 border-x border-primary/20' : ''}`}>
                        {typeof val === 'boolean' ? (
                          val ? (
                            <CheckCircle size={18} className="text-green-600 mx-auto" weight="fill" />
                          ) : (
        ...
        <div className="container">
          ...
          <div className="space-y-2">
            {pricingFaqs.map((faq, i) => (
              <div key={faq.q} className="border border-border rounded-xl overflow-hidden bg-background">
                <Button
                  variant="ghost"
                  onClick={() => setOpenFaq(openFaq === i ? null : i)}
                  className="w-full text-left p-5 flex items-center justify-between gap-4 rounded-none hover:text-primary"
                >
      ...
      <section className="section-padding-md bg-primary text-primary-foreground">
        <div className="container text-center max-w-2xl mx-auto">
          ...
          <p className="text-primary-foreground/70 mb-8">
            Volume besar, kebutuhan kustom, SLA ketat? Kami punya solusinya. Hubungi tim sales kami sekarang.
          </p>
          <Button asChild variant="inverted" size="lg" className="inline-flex items-center gap-2">
            <Link href="/contact">
