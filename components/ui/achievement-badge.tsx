...
export function AchievementBadge({
  ...
  const showProgress = !earned && progress != null
  ...
  return (
    <Card onPress={onPress} accessibilityLabel={a11y} className={cn("items-center gap-3", className)} {...rest}>
      <View className="relative">
        <IconBox icon={icon ?? FALLBACK_BADGE_ICON} size="xl" variant={earned ? "inverted" : "surface"} shape="circle" weight={earned ? "fill" : "regular"} />
        {!earned ? (
      ...
      <View className="items-center gap-0.5">
        <Text variant="label" tone={earned ? "primary" : "disabled"} numberOfLines={1} className="text-center">
          {name}
        </Text>
        {description ? (
          <Text variant="caption" tone={earned ? "secondary" : "disabled"} numberOfLines={2} className="text-center">
            {description}
          </Text>
        ) : null}
      </View>
...
export function AchievementBadgeGrid({ items, onPressItem, labels, className, ...rest }: AchievementBadgeGridProps) {
  return (
      ...
      {items.map((item) => {
        ...
        return (
          <GridItem key={id} span={6}>
            <AchievementBadge {...badge} labels={labels} onPress={onPressItem ? () => onPressItem(item) : undefined} className="h-full" />
          </GridItem>
        )
      })}
    </Grid>
