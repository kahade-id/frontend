/**
 * Kahade — <DeliveryProofViewer> (§9.6 Card, §9.7 Badge, §5 radius foto,
 * §13 format tanggal, §12 Voice & Tone, §10 konfirmasi destruktif = Dialog).
 *
 * Menampilkan bukti pengiriman yang diunggah penjual
 * (`GET /v1/orders/{id}/delivery-proof`): foto/berkas, catatan, nomor resi,
 * dan waktu unggah. Bagi PEMBELI, komponen ini juga membawa dua aksi
 * penentu escrow — "Konfirmasi diterima" (dana dilepas ke penjual) dan
 * "Tolak" — sehingga hierarki visual dibuat agar pembeli MELIHAT bukti dulu
 * sebelum bisa memutuskan.
 *
 * Anatomi: header (judul + Badge status) -> galeri foto (1 besar, sisanya
 * grid 3 kolom) -> resi (Mono, bisa disalin) -> catatan penjual -> waktu
 * -> aksi pembeli (bila `viewer="buyer"` dan status masih menunggu).
 *
 ...
 *   - Tombol pembeli: "Konfirmasi diterima" `primary`, "Tolak" `ghost` —
 *     BUKAN `destructive`. Menolak bukti tidak menghapus apa pun; ia
 *     membuka sengketa. Pemanggil WAJIB menampilkan Dialog konfirmasi (§10)
 *     untuk KEDUA aksi karena keduanya menggerakkan dana escrow; komponen
 *     hanya memanggil `onConfirm`/`onReject`.
 *   - Status memakai Badge dari tone semantik: menunggu = warning, diterima
 *     = success, ditolak = danger. Alasan penolakan (`rejectionReason`)
 *     tampil dalam <Alert tone="danger" variant="soft"> karena ini fakta
 *     transaksi yang harus terbaca penjual, bukan hiasan.
