...
| **Dipakai, tapi di luar jangkauan auditor** | `POST /v1/auth/refresh` | ✅ **DITUTUP (API-20)** — kini terdaftar eksplisit di `audit-inventory` sebagai jalur di luar `http.*`, diverifikasi lewat probe; `check:api` mencetaknya setiap run |
...
| **API-20** | **Blind spot tooling:** `audit-inventory.mjs` hanya memindai pemanggilan `http.*` di `lib/api/`. Dua jalur HTTP nyata tak terlihat: `refreshAccessToken()` (`exchange()` langsung) dan `uploadToPresignedUrl()` (`fetch()` langsung ke object storage) | `scripts/audit-inventory.mjs:28-45` | ✅ **DITUTUP** — keduanya kini terdaftar di konstanta `OUT_OF_BAND_HTTP`, masing-masing dengan `probe` regex yang harus masih cocok di berkasnya. Bila kode di-refactor dan probe tak lagi cocok, `check:api` gagal dengan `OUT_OF_BAND_STALE`, jadi daftarnya tidak bisa basi diam-diam. Terverifikasi dua arah: merusak `REFRESH_PATH` → EXIT=1; dipulihkan → EXIT=0. |
