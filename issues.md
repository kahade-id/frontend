...
| E — domain waktu & tenggat | `02d237e` | E-01…E-08 |
| F — aksesibilitas & label dinamis | `02d237e` | F-01…F-10 |
| G — i18n & lokalisasi | `4833fde`, `750b486` | G-01…G-09 |
| H — komponen UI & antrean toast | `3d096b9` | H-01…H-14 |
| I — bersih-bersih & ratchet gate | `86ee1a9`, `48b6aa5` | I-01…I-15 |
| J — build, konfigurasi & rilis | remediation | J-01…J-12 |
| K — testing, CI & kontrak validasi | remediation | K-01…K-09 |
| L — performa & latensi jaringan | remediation | L-01…L-07 |
...
## Catatan batch G — i18n & lokalisasi (G-01…G-09)

- **G-01/G-02** — string dinamis dan template literal di accessibility label
  dikeluarkan ke `translate("… {x} …", { x })`.
- **G-03…G-09** — katalog wajib 100% diterjemahkan, kognat diverifikasi
  eksplisit, WIB disesuaikan per bahasa (WIB di ID, UTC+7 di EN).

## Catatan batch H — komponen UI & antrean toast (H-01…H-14)

- **H-01** — plafon antrean `ToastProvider` dibatasi agar banjir error tidak
  membanjiri layar; toast lama dibersihkan.
- **H-02** — `AmountKeypad` dibatasi pada `Number.isSafeInteger` untuk
  mencegah overflow integer JavaScript.
- **H-03…H-14** — pembersihan timer per alur, perbaikan gesture PanResponder,
  dan sinkronisasi padding BottomSheet & class merge `cn()`.

## Catatan batch I — dead code & ratchet plafon (I-01…I-15)

- **I-01** — ratchet plafon komponen UI tak terpakai (S5) dikunci di 29.
- **I-06** — unifikasi alasan laporan ke satu sumber di `lib/labels/report.ts`
  (menghilangkan duplikasi `REPORT_REASONS` di `report-form.tsx`).
- **I-07/I-08** — penyatuan status labels ke modul label terpusat.
- **I-09** — penguncian 16 god components > 700 baris dengan plafon baris ketat.
- **I-10** — ratchet plafon aturan S1 & S3 di `check-screens.mjs`.
- **I-13…I-15** — pembersihan aset dummy, penyesuaian `tsconfig.json`, dan
  dokumentasi npm.

## Catatan batch J — build, konfigurasi & rilis (J-01…J-12)

- **J-02** — deklarasi `enableBackgroundRemoteNotifications: true` dan
  `UIBackgroundModes: ["remote-notification"]` dihapus dari `app.json` karena
  tidak ada background task manager yang terdaftar.
- **J-04** — versi aplikasi disinkronkan: `package.json` dinaikkan ke `1.0.0`
  menyamai `app.json`.
- **J-07** — `public/manifest.json` diselaraskan orientasinya menjadi
  `"portrait"` menyamai build native, dan ditambahkan `shortcuts` untuk alur
  utama (Tarik Dana, Transfer Saldo, Terima Saldo).
- **J-08** — skrip rollback OTA darurat dibuat di `scripts/ota-rollback.mjs`
  dan didaftarkan sebagai `npm run ota:rollback`.
- **J-09** — `EXPO_PUBLIC_API_ENV` didokumentasikan di `.env.example`.

## Catatan batch K — testing & verifikasi kontrak (K-01…K-09)

- **K-01** — test unit komprehensif ditambahkan untuk modul murni dan state:
  - `tests/pull-math.test.ts` (12 test)
  - `tests/server-time.test.ts` (5 test)
  - `tests/pending-actions.test.ts` (6 test)
- **K-01/E-07** — perbaikan bug `resetDrift()` pada `resetServerTime()` di
  `lib/server-time.ts`.
- **K-08** — eliminasi kondisi mati `serial > 9999` di `lib/nik.ts` dan
  penambahan test suite `tests/nik.test.ts` (9 test) untuk validasi NIK.
- **K-09** — penambahan unit test di `tests/format.test.ts` untuk
  `formatCountdown`, `durationHoursParts`, dan `formatDateLong`.

## Catatan batch L — performa & latensi jaringan (L-01…L-07)

- **L-02** — optimasi `mergeById` di `lib/use-paginated-query.ts` dengan
  fast-return saat salah satu array kosong untuk menghemat alokasi Map.
- **L-05/L-06** — memoize `cachedAppVersion` dan `cachedDeviceInfo` di
  `lib/api/session.ts` untuk mengeliminasi overhead I/O berulang pada setiap
  HTTP request.
- **L-07** — penambahan `timeoutMs: 60_000` untuk endpoint ekspor dompet
  (`exportWalletCsv`, `exportWalletPdf`) di `lib/api/wallet.ts`.
